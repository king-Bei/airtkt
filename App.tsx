...
import { flightService } from './services/flightService'; // Added import
...
const App: React.FC = () => {
  ...
  const handleFinalBookingSubmit = async (passengers: Passenger[]) => {
    if (!currentUser || !outboundFlight) return;

    // 改為呼叫真實/Mock GDS 進行訂位
    let pnr = "PENDING";
    try {
      setLoading(true); // 顯示讀取中
      const gdsPnr = await flightService.createBooking(outboundFlight, passengers, returnFlight || undefined);
      if (gdsPnr) pnr = gdsPnr;
    } catch (e) {
      alert("GDS 訂位失敗: " + e);
      setLoading(false);
      return;
